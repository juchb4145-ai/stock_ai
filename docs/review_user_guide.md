...
| 단계  | 명령어                                                                 | 모드      | 변경 발생?             |
| --- | ------------------------------------------------------------------- | ------- | ------------------ |
| 1   | `python fetch_minute_bars.py 2026-04-30`                            | 캐시 수집   | data/intraday 만 갱신 |
| 2   | `.\.venv\Scripts\python.exe .\analyze_today.py 2026-04-30`          | 일별 리뷰   | data/reviews 만 갱신  |
| 3   | `.\.venv\Scripts\python.exe -m review.rolling 2026-04-30`           | 누적 통계   | data/reviews 만 갱신  |
| 4   | `.\.venv\Scripts\python.exe apply_overrides.py 2026-04-30`          | dry_run | 없음 (감사 로그만)        |
| 5   | (사람이) `data/reviews/rule_candidates_20260430.json` 편집               | 검토/승인   | JSON 1줄 수정         |
| 6   | `.\.venv\Scripts\python.exe apply_overrides.py 2026-04-30 --commit` | commit  | **모듈 상수 변경**       |
...
| confidence | 표본 수 (윈도우 전체 거래) | 일관성                  | commit 적용 가능?                                      |
| ---------- | ---------------- | -------------------- | -------------------------------------------------- |
...
- `data/reviews/rolling_summary_YYYYMMDD.json` 의 confidence 확인
