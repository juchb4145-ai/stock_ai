"""키움 OpenAPI FID 코드 ↔ 한글 이름 매핑.

main.py 가 OnReceiveChejanData / OnReceiveRealData 응답을 dict 로 매핑할 때
사용한다. 이 표는 키움 OpenAPI 가이드 PDF 의 실시간/체결 FID 정의를 그대로
옮긴 것이며, 거래 로직과 분리되어 있어 별도 모듈로 둔다.

사용 예:
    from fid_codes import FID_CODES, get_fid
    name = FID_CODES["228"]          # "체결강도"
    code = get_fid("체결강도")        # "228"
"""
...
def get_fid(fid_name: str) -> str:
    """한글 FID 이름으로 숫자 FID 를 돌려준다."""
    try:
