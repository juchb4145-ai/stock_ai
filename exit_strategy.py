...
매도 정책:
  1. 손절: 현재가 ≤ stop_price → 전량 매도. 퀀트조건식은 -1.5% 고정 손절.
  2. +1R 도달 → stop_price 를 entry_price 로 상향(BE 보장).
  3. 퀀트조건식은 +2% 도달 시 전량 익절. 기존 R 전략은 +2R 부분 익절.
...
def evaluate_exit(ctx: ExitContext) -> ExitDecision:
    ...
    # 1) Requested hard exits for the safe pullback strategy.
    safe_pullback_position = getattr(pos, "breakout_grade", "") in ("SAFE", "QUANT")
    if safe_pullback_position:
        if profit_rate >= FIXED_TAKE_PROFIT_PCT:
            return ExitDecision(
                "sell",
                1.0,
                "+2% 전량 익절 ({:.2%})".format(profit_rate),
            )
        if profit_rate <= -FIXED_STOP_LOSS_PCT:
            return ExitDecision(
                "sell",
                1.0,
                "-1.5% 고정 손절 ({:.2%})".format(profit_rate),
            )
        if ctx.five_min_ind:
            sma20_prev = getattr(ctx.five_min_ind, "sma20_prev", None)
            prev_close = getattr(ctx.five_min_ind, "prev_close", None)
            if sma20_prev and prev_close and prev_close < sma20_prev * (1 - FIVE_MIN_MA_BREAK_BUFFER_PCT):
                return ExitDecision(
                    "sell",
                    1.0,
                    "5분봉 20선 종가 이탈 (종가 {:.0f} < MA20 {:.0f})".format(prev_close, sma20_prev),
                )
