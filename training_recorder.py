...
TRADE_LOG_CONDITION_RULES = (
    "A: [5분]0봉전 Bollinger Band(45,2) 종가가 상한선 상향돌파 | "
    "B: [5분]0봉전 Bollinger Band(55,2) 종가가 상한선 상향돌파 | "
    "C: [5분]0봉전 Envelope(13,2.5) 종가가 Envelope 상한선 상향돌파 | "
    "D: [5분]0봉전 Envelope(22,2.5) 종가가 Envelope 상한선 상향돌파 | "
    "E: [5분]1봉전 Envelope(13,2.5) 종가가 Envelope 상한선 상향돌파 | "
    "F: [5분]1봉전 Envelope(22,2.5) 종가가 Envelope 상한선 상향돌파 | "
    "G: [5분]1봉전 Bollinger Band(45,2) 종가가 상한선 상향돌파 | "
    "H: [5분]1봉전 Bollinger Band(55,2) 종가가 상한선 상향돌파 | "
    "I: [일]거래량:50000이상 999999999이하 | "
    "J: 체결강도 100.0%이상 1000.0%이하 | "
    "N: [일]거래대금(일:백만, 분:천) 2000이상 999999999이하 | "
    "O: 거래량비율:[5분]20봉 평균(0봉전)거래량대비 동일주기 0봉전 200%이상 | "
...
def _attach_first_pullback_quality_fields(
    row: Dict[str, Any],
    ctx: Any,
    decision: Any,
) -> None:
    for field in FIRST_PULLBACK_QUALITY_FIELDS:
        row[field] = (
            getattr(decision, field, "")
            if getattr(decision, field, "") not in ("", None)
            else getattr(ctx, field, "")
        )
...
class TrainingRecorderMixin:
    ...
    def register_dante_training_sample(
        ...
        *,
        ...
        _attach_first_pullback_quality_fields(row, ctx, decision)
        row.update(features)
